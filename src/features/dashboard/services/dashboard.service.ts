...
export async function getAllOrders(userRoles: RoleName[] = [], currentUserId?: string) {
  return sharedGetAllOrders(userRoles, currentUserId);
}
 ...
 * Wrapper around shared service for dashboard-specific needs
 */
export async function updateOrderStatus(orderId: string, status: string, userRoles: RoleName[] = [], userId?: string) {
  return sharedUpdateOrderStatus(orderId, status, userRoles, userId);
}
